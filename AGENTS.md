...
3. **기능 완료 시** `docs/projects/current/features.json`의 status를 "done"으로 업데이트.
...
8. **프론트엔드 기능 구현 시 CORS 미들웨어와 DB 시드 등 통합 필수조건을 확인하라.**
9. **모든 기능 완료 후, 실제 서비스를 기동하여 e2e 스모크 테스트를 1회 이상 수행하라.** 테스트 통과만으로는 부족하다.
