...
### 다층 원인 분석 구조 (project-004)
**선택**: AI 분석 결과를 3단계(직접/간접/거시)로 분류하여 JSONB에 저장
**대안**: 단일 causes[] 배열 (기존 방식)
**이유**: 급변동 원인이 단일하지 않음. 직접 원인(실적), 간접 원인(공급망), 거시 요인(금리)을 구분해야 사용자에게 유의미한 분석 제공
**트레이드오프**: 기존 causes[] 형식과 호환성 유지 필요 (프론트엔드에서 graceful fallback)

### E2E 파이프라인 — Celery Chain (project-004)
**선택**: 가격수집→급변동감지→뉴스수집→AI분석→알림을 Celery chain으로 연결
**대안**: 각 단계를 독립 스케줄로 실행
**이유**: 급변동 감지부터 리포트 완성까지 인과관계가 명확. chain으로 연결하되 각 단계의 실패가 다음 단계로 전파되지 않도록 에러 격리
**트레이드오프**: chain 중간 실패 시 부분 결과만 남을 수 있음

### Alembic 마이그레이션 도입 (project-004)
**선택**: Alembic으로 DB 스키마 마이그레이션 관리
**대안**: create_tables()만 사용 (기존 방식)
**이유**: 운영 중 스키마 변경(컬럼 추가/수정)을 안전하게 관리. autogenerate로 마이그레이션 자동 생성
**주의**: create_tables()와 이중 관리 상태. 장기적으로 Alembic 단일 경로로 통합 필요
